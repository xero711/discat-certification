...
認証ページは `identify` スコープでDiscordへ移動し、戻ってきた `code` と `guild_id` をGuard APIの `/verify/oauth/complete` に送ります。Client Secret はGitHub Pagesには置かず、Guard API側の `config.py` だけで使います。
